...
class FastGPT2ForSequenceClassification(GPT2PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"h\.\d+\.attn\.masked_bias", r"lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.transformer = FastGPT2Model(config)
...
class FastGPT2Model(GPT2PreTrainedModel):
    _keys_to_ignore_on_load_missing = ["attn.masked_bias"]

    def __init__(self, config):
        super().__init__(config)
        # self.config= GPT2Config.from_json_file("gpt2/config.json")
        ...
        self.h = nn.ModuleList(
            [
                FastGPT2Block(config, layer_idx=i)
