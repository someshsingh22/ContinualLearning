...
from transformers import BertTokenizer, BertForMaskedLM, BertModel, BertPreTrainedModel
...
class SlowLearner(nn.Module):
    ...
    def forward(self, input_id, mask):
        _,h, x= self.bert(input_ids= input_id, attention_mask=mask,return_dict=False)
        x = self.relu(x)
...
class FastLearner(nn.Module):
    def __init__(self, h,out):
        ...
        self.bert = FastBertModel.from_pretrained('bert-base-cased')
        self.linear = nn.Linear(h,out)
        self.relu = nn.ReLU()
    ...
    def forward(self, input_id, mask):
        ...
        return x
class FastBertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = FastBertEncoder(config)
