...
parser.add_argument("--xdim", type=tuple, default=(3, 32, 32), help="Input Dimensions")
...
if __name__ == "__main__":
    ...
    train_taskset = TaskDataset(
        train_data, train_transforms, num_tasks=args.n_class // args.n_ways
    )
    test_transforms = [
        ContinousNWays(test_data, args.n_ways, args),
        l2l.data.transforms.LoadData(test_data),
    ]
    test_taskset = TaskDataset(
        test_data, test_transforms, num_tasks=args.n_class // args.n_ways
    )
    ...
    with tqdm(
        ...
        for run in pbar:
            ...
            with tqdm(
                enumerate(train_taskset),
                desc="Task Loop",
                total=args.n_tasks,
                leave=False,
                position=0,
            ) as outer:
                for task, task_data in outer:
                    train_loader = DataLoader(
                        TensorDataset(*task_data),
